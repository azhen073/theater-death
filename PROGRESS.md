...
| 里程碑 | 状态 | 说明 |
| --- | --- | --- |
| 文档 | ✅ | 规则书 v1.1（含 2026-09-19 追加：Q-09 移交时机 + 规则 2.0 命名预设；合并版见 `docs/rules-v2-full.md`）+ 需求文档 **v2.0.2-beta**，Q-01–Q-08 全量定值（规则书第 09 章） |
...
| 2.0.2-beta（遗言系统复核 + 账户显示设置 F2，贡献 kiahir） | 🚧 | **遗言**：链路逐条静态复核（R-41/R-45/R-46 与 V2-01/V2-02，结论：实现与规则一致）；**G5** 把「同日多名出局者按座位号升序依次遗言、每人各自 60 秒」写入规则书 R-45 与 `docs/rules-v2-full.md`（**无行为变更**，属记录缺口）；**G4** 新增 `e2e/specs-v2/17-last-words.spec.ts`（遗言者「结束遗言」+公屏可发；活人旁观者公屏可发但无遗言行动、另一名死者公屏只读）——**chromium 2/2、webkit 2/2 实测通过**。**显示与动画**：三设置（动画偏好/死亡特效/界面缩放）实现核对通过；**F2 已修**——「死亡特效」复选框补 `aria-label`（原先可访问名把帮助文本读进去），`08-display` 三处定位收紧为按名匹配；**F5 已修 + 字号统一**——该区块加 `display-settings` 类定点收口：三标签统一 14px、帮助文字 12px、死亡特效行补 18px 下边距（原先紧贴「界面缩放」）；`01-account` 补账号页断言（区块可达 + 默认值 + 改动即时生效 + 字号/间距回归 + 截图）——**8/8 实测通过**。未决：F1 账号页 `!saved` 分支、F3 `saved` 初值、F4 缺 display 文档；`lastWords.firstNight`/`otherNights` 为死配置；麦克风许可仅单测层覆盖 |

## 接续指引（compact 后先读这里）

1. 读本文件 + `AGENTS.md`（项目规则与 Docker 约束）即可接上状态。
2. 规则细节查 `theater_death_rulebook_v1.1.md`（第 09 章 = S3 裁定）；
   工程规格查 `theater_death_development_requirements_v1.1.md`（最新 v2.0.2-beta（进行中）：v1.6 声网替换 · v1.7 规则 2.0 与移交时机 · v1.8 账号与 v2 体系准入 · v1.9 文档一致性整理 · v2.0.1-beta 解散与遗弃回收 · v2.0.2-alpha 舞台行动 UX 整合（= 2.0.2-beta 内容基线）· v2.0.2-beta 本分支新改动，见文末版本记录）。
