...
| 2.0.3-alpha（后续改动，贡献 kiahir） | 🚧 | 分支自 `2.0.2-beta` 的 tip（`c5df078`）拉出，2.0.2-beta 全部内容即其基线。计划：**局内语音音量显示与调节**（显示方案 A/A+B/A+B+C 待定；调节含输出音量+一键静音、输入增益、可选每玩家音量；纯本地信号与偏好、不上报、受 R-43 时段门控、尊重减少动画、不改规则语义；本地静音不改变发言权与"谁在发言"的指示）+ 上一版遗留 F1/F3/F4、P1/P2 |
