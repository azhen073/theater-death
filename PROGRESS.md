...
| 2.0.4-beta（声网媒体回收可靠性，贡献 kiahir） | 🚧 | 分支自 `main` 的 `9c86a90` 拉出（版本名 = 分支名），**进行中**。声网语音审计后的可靠性修复（无玩法/契约字段变更）：`server/v2/media.ts` 踢出改为**成功后才删身份映射**、失败留待下一次 `sync` 重试（官方明确一次性踢出在断连时会失败），`sync` 逐身份捕获失败并统计；`enqueue` 队列链**永不 reject**（修掉 `required` 失败后下一次 work 被静默跳过）；`voice/agora.ts` REST 加 `AbortSignal.timeout`（默认 15s）+ 5xx/超时/网络错误退避重试（默认 3 次尝试），4xx 不重试；`voice/agora.ts` 发布凭证默认 **TTL 600s → 150s**（覆盖最长 120s 发言窗口 + 30s 缓冲，收口「被改客户端在窗口外仍可发麦」的最坏时长；方案②「失去发布权即踢 uid」本轮不做）。实测：增量 **5 文件 40 例全过** + 服务端 `typecheck` 通过；**旧实现 stash 对照验证**两条新用例确实失败。静态清点 88 文件 578 例。审计另发现 10 项未处理，见需求文档文末 v2.0.4-beta |
