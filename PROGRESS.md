...
| 2.0.4-beta（声网媒体回收可靠性，贡献 kiahir） | 🚧 | 分支自 `main` 的 `9c86a90` 拉出（版本名 = 分支名），**进行中**。声网语音审计后的可靠性修复（无玩法/契约字段变更）：`server/v2/media.ts` 踢出改为**成功后才删身份映射**、失败留待下一次 `sync` 重试（官方明确一次性踢出在断连时会失败），`sync` 逐身份捕获失败并统计；`enqueue` 队列链**永不 reject**（修掉 `required` 失败后下一次 work 被静默跳过）；`voice/agora.ts` REST 加 `AbortSignal.timeout`（默认 15s）+ 5xx/超时/网络错误退避重试（默认 3 次尝试），4xx 不重试；`voice/agora.ts` 发布凭证默认 **TTL 600s → 150s**（覆盖最长 120s 发言窗口 + 30s 缓冲，收口「被改客户端在窗口外仍可发麦」的最坏时长；方案②「失去发布权即踢 uid」本轮不做）。实测：增量 **5 文件 40 例全过** + 服务端 `typecheck` 通过；**旧实现 stash 对照验证**两条新用例确实失败。另**收口客户端连接可靠性 5 类**（补 `token-privilege-will-expire/did-expire` 续期与过期重连、修 autoMic 窗口键在重连中被消耗、重连期间开麦按钮禁用+「重连中…」、`#stopTask` 串行化关麦重开、uid 归属/`exception` 提示/离开复位自动播放回调/设备保留）：单测 **19 例全过** + `typecheck:web:v2`/`v2-tests`/`web`(v1) 三工程通过 + E2E `18-voice-levels` **双浏览器 3/3**，同样用 stash 旧实现验证过（session 6 failed / bar 1 failed）。静态清点 **88 文件 584 例**、v2 E2E 静态 **18 spec / 53 例**。仍余 1 条未做：v1 无自动重连且 uid = 座位号（多标签页 `UID_CONFLICT`）；**声网真机验收通过（2026-09-21，本机 `.env` 真实凭据）**：`16-voice` **1 passed（12.3s）**——13 客户端真机入频道、候选开麦发布、接收方远端电平 **35–42%（真实收流）**、结束发言即时撤权；顺带修掉该 spec 两处自声网迁移起就坏掉的缺陷（文案定位器被 v2.0.3-alpha 的 `.voice-bar__speaker` 撞成 strict 双命中；`<audio>` 元素断言在 WebAudio 播放模式下根本不成立）。**另实测：本项目「连麦鉴权」未开启 → 订阅 token 也能发麦（部署前必须先启用，详见「测试状态」）**。v1 `02-voice` 仍需控制台客户 ID/密钥，未跑；审计另发现 9 项未处理，见需求文档文末 v2.0.4-beta |

## 接续指引（compact 后先读这里）

1. 读本文件 + `AGENTS.md`（项目规则与 Docker 约束）即可接上状态。
2. 规则细节查 `theater_death_rulebook_v1.1.md`（第 09 章 = S3 裁定）；
   工程规格查 `theater_death_development_requirements_v1.1.md`（最新 v2.0.4-beta：v1.6 声网替换 · v1.7 规则 2.0 与移交时机 · v1.8 账号与 v2 体系准入 · v1.9 文档一致性整理 · v2.0.1-beta 解散与遗弃回收 · v2.0.2-alpha 舞台行动 UX 整合 · v2.0.2-beta 遗言顺序明文 + 账户显示设置修复 + 文档审计 · v2.0.3-alpha 局内语音音量显示与调节 · v2.0.4-alpha 竞选投票资格修正 + 夜间公开时钟 + 死神知识呈现 + 语音自动化 · v2.0.4-beta 声网媒体回收可靠性 + 撤销强度待定案，见文末版本记录）。
...
2.0.4-beta 增量（2026-09-21，贡献 kiahir）：**5 文件 40 例全过**（`voice-agora` 10 例、`v2-media` 9 例、`voice-policy` 10 例、`voice-api` 7 例、`v2-voice-api` 4 例）+ 服务端 `npm run typecheck` 通过；新增 6 例后静态清点为 **88 文件 578 例**（**本分支未跑全量**，上面那条 572 是 `main` 的实测记录）。两条媒体回收回归用例已用「临时把 `server/v2/media.ts` stash 回旧实现」对照验证：旧实现下 **2 failed**、新实现全绿。

2.0.4-beta 客户端增量（2026-09-21，贡献 kiahir）：**2 文件 19 例全过**（`frontend-v2-voice-session` 13 例、`frontend-v2-voice-levels` 6 例；新增 6 例覆盖凭证续期/过期重连恢复开麦/质量异常提示/关麦重开不双发/uid 归属/设备保留）+ `typecheck:web:v2`、`typecheck:web:v2-tests`、`typecheck:web`（v1 前端）三个工程通过 + 界面 E2E `e2e/specs-v2/18-voice-levels.spec.ts` **chromium 3/3 + webkit 3/3**（acceptance 栈 + seed，跑完 `down -v` 收栈）。静态清点更新为 **88 文件 584 例**，v2 E2E 静态 **18 spec / 53 例**。回归有效性：旧 `session.ts` → **6 failed / 7 passed**；旧 `bar.tsx` → 新 E2E 用例 **1 failed**。

声网真机验收（2026-09-21，v2 入口，本机 `.env` 真实凭据）：`e2e/specs-v2/16-voice.spec.ts` **chromium 1/1 通过（12.3s）** —— 13 个客户端真机加入声网频道、候选开麦发布、**接收方远端电平 35–42%（真实收到音频流）**、无自动播放拦截、结束发言后发布权即时收回；截图 `test-results-frontend-v2/voice-{joined-listener,speaking,permission-revoked}.png`。跑完 `down -v` 收栈。**v1 `02-voice` 仍需控制台「客户 ID/密钥」，未跑**；麦克风增益/AGC 的听感与踢人/关房的真实 REST 也未验。

声网鉴权强制力探针（2026-09-21，真实项目，临时用例验证后已删）：**加入频道强制校验 token**（无 token → `dynamic use static key`；错误 App 证书 → `invalid token, authorized failed`）✅；但**发布权限位当前无约束力**——持订阅角色 token 在 **rtc 与 live** 两种 mode 下都**成功 `publish`**，且 `pubAudio` 仅 20 秒的发布凭证 35 秒后仍能再次发麦 → 按官方文档判定该项目**「连麦鉴权」尚未开启**。**部署前必须在控制台「项目 → 编辑 → 功能 → 连麦鉴权」启用**（约 5 分钟生效），开启后重跑探针应看到订阅 token 的 publish 变为 rejected。在此之前的实际保障：R-43 发布权只由服务端签发/撤回（依赖客户端 `renewToken`）+ 发布 TTL 150 秒上限。
