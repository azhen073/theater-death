...
| 2.0.4-beta（声网媒体回收可靠性，贡献 kiahir） | 🚧 | 分支自 `main` 的 `9c86a90` 拉出（版本名 = 分支名），**进行中**。声网语音审计后的可靠性修复（无玩法/契约字段变更）：`server/v2/media.ts` 踢出改为**成功后才删身份映射**、失败留待下一次 `sync` 重试（官方明确一次性踢出在断连时会失败），`sync` 逐身份捕获失败并统计；`enqueue` 队列链**永不 reject**（修掉 `required` 失败后下一次 work 被静默跳过）；`voice/agora.ts` REST 加 `AbortSignal.timeout`（默认 15s）+ 5xx/超时/网络错误退避重试（默认 3 次尝试），4xx 不重试。实测：增量 **5 文件 39 例全过** + 服务端 `typecheck` 通过；**旧实现 stash 对照验证**两条新用例确实失败。静态清点 88 文件 577 例。待定案：撤销强度（发布 TTL 600s 内被改客户端仍可发麦）；审计另发现 10 项未处理，见需求文档文末 v2.0.4-beta |
...
2.0.4-beta 增量（2026-09-21，贡献 kiahir）：**5 文件 39 例全过**（`voice-agora` 9 例、`v2-media` 9 例、`voice-policy` 10 例、`voice-api` 7 例、`v2-voice-api` 4 例）+ 服务端 `npm run typecheck` 通过；新增 5 例后静态清点为 **88 文件 577 例**（**本分支未跑全量**，上面那条 572 是 `main` 的实测记录）。两条媒体回收回归用例已用「临时把 `server/v2/media.ts` stash 回旧实现」对照验证：旧实现下 **2 failed**、新实现全绿。
