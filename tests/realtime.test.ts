...
describe('实时通道：握手与推送', () => {
  ...
  it('开局推送：公共事件广播全员，身份只发本人', async () => {
    ...
    await waitFor(() => socketA.gameEvents.some((event) => event.type === 'game_started'));
    expect(socketB.gameEvents.some((event) => event.type === 'game_started')).toBe(true);

    await waitFor(() => socketA.gameEvents.some((event) => event.type === 'role_assigned'));
