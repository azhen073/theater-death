...
### v2.0.2-beta（2026-09-21）· 遗言顺序明文 + 账户显示设置修复 + 文档审计（贡献 kiahir，用户（阿真）确认；分支 `2.0.2-beta` 已推送至远端，待 PR 合并）
...
- **版本号**：沿用「版本名 = 分支名」约定，本版 = **`2.0.3-alpha`**。分支自上一版 `2.0.2-beta` 的 tip（`c5df078`）拉出，因此 **2.0.2-beta 的全部内容（遗言 G4/G5、账户显示 F2/F5、全量 md 审计）即本版基线**；与规则版本 2.0、客户端契约 2.1/2.2 仍是三套独立编号；文件名保留基线名 `theater_death_development_requirements_v1.1.md`。
- **计划内容（实现后在此补记实测与未覆盖项）**：
  - **局内语音：音量显示 + 音量调节**（方案已提交待定案）：
    - **显示**：A / A+B / A+B+C 三档——A 只显示自己的电平；B 增加「当前发言者」电平；C 为 13 座位全员电平环（需服务端给出 uid↔座位映射，涉及契约与夹具）。
    - **调节**：①**输出音量**（远端播放，0–100 + 一键静音本机输出）；②**输入增益**（自己麦克风采集音量，0–100）；③可选**每玩家单独音量**（含"本地静音某人"，仅影响本机播放）。
    - 共同约束：**音量为纯本地信号与本地偏好，不上报服务端、不入日志与复盘**；受 R-43 许可时段门控（投票/夜间等窗口不显示）；尊重显示设置里的「减少动画」；**不改变 R-43 与 R-35 的任何语义**；本地静音**不改变**服务端的发言权与计时，界面上仍须保留"谁在发言"的指示（避免误以为无人说话）；不允许调控他人麦克风（那属服务端权限，R-43 管辖）。
    - 持久化：与显示偏好同源（`localStorage`，浏览器级），缺省回默认、不更换存储键；UI 落在语音条（输出/静音，麦开时给增益）与「我的账户」设置区。
    - 候选 API 与实现要点（**落地前需在容器内核对 SDK typings**）：远端每用户 `audioTrack.setVolume(0–100)`；本机麦克风轨道增益 `micTrack.setVolume(...)`（范围与 AGC 交互需实测，且**每次重新开麦的新轨道都要重新应用**）；音量指示应在**增益之后**取值，输出音量不影响指示。
