...
白天时序、竞选、发言轮、放逐投票、平票、遗言、票型公示与天理移交均按规则书 R-41–R-46 定值实现，不得自行改用某线下版本细则。天理票权 1.5 以整数定点结算（普通票 2 单位、天理票 3 单位）。**遗言队列**：同一节点有多名出局者时按座位号升序依次开启，每人各自一整段 60 秒窗口（`engine/day.ts` 的 `lastWords.queue` 排序；驱动按人重开窗口，超时或本人 `END_LAST_WORDS` 后推进下一位）。
...
| T-39 | 首夜死者 / 白天出局者 / 后续夜晚死者 | 前二者各获 60 秒遗言（多人同日时按座位号升序依序进行，每人各自 60 秒）；后续夜晚死者无遗言；被还魂曲救回者无遗言。 |
...
### v2.0.6-alpha（2026-09-22，进行中；原分支 `2.0.4-beta` 同日改名）· 声网语音可靠性收口（媒体回收 / 发布 TTL / 客户端）+ 真机验收与鉴权实测（贡献 kiahir，用户（阿真）确认）
...
#### v2.0.6-alpha 追加（2026-09-23，贡献 kiahir）· 送达回执（C）+ 频道轮询对账（D）+ 发言者电平修正

用户要求「解决声音数据无法传输至服务端」，方案与逐条官方核对见 `docs/proposal-voice-delivery-observability.md`。原提案的 A/B（服务端广播统计、客户端误报）经复核放弃改做 C/D：**不做「服务端能听见什么」，改做「接收侧真的收到没有」+「谁该在频道里却不在」**。

- **C 端到端送达回执（无新玩法语义）**：新增 `POST /api/v2/rooms/:code/voice/receipt`——意图型接口，`limited(60/60s)` + `directory.member` + 对局校验，身份一律取 `room.access.resolve(s)`（不信任载荷自报），`windowInstanceId` 与 `state` 均校验（错误码 `invalid_window_instance_id`/`invalid_request_payload`）；仅发言窗口（`election_speech`/`speech_round`/`last_words`/`tie_speech`）内受理，且**拒绝发言者本人的回执**与非授权身份。客户端 `web-v2/.../voice/session.ts` 在订阅成功 / 输出静音 / 自动播放被拦 / 发布失败时上报（1 秒节流，按窗口+状态去重，断线期间不报）。快照把 `private.voice.delivery` **只发给当前发言者**（`!readOnly && caps.canPublishVoice`），公开视图永不含该字段（`tests/v2-voice-delivery-api.test.ts` 有专门断言）。界面：麦克风开启且听众 >0 时在语音条内以 **10px 灰字**显示「已送达 N/M」，无回执时「等待接收确认」，听众为 0 不显示。
- **D 频道轮询对账（不使用 NCS）**：`voice/agora.ts` 新增**可选** `queryChannelUsers(roomName)`（客户 ID/密钥 Basic 认证，走既有超时+退避重试；对 `channel_exist`/`users` 缺失做容忍解析）。`server/v2/media.ts` 的 `reconcile()` 每 **5 秒**（`maintenance.ts` 的 `ReconcileOptions`，**独立于游戏队列**，失败只计数）仅对**进行中**对局比对「该在频道里的身份」与频道实况：**只踢未知 uid**（听众合法在线，不得误踢；每个 round 上限 3 次），统计 `rounds/notInChannel/unknownKicks/failures/skipped`，经 `ADMIN_PASSWORD` 的 `AdminSummary.voice.reconcile` 暴露。未配凭据时 `queryChannelUsers` 不存在 → 整体 `skipped`（不报错）。
- **发言者电平修正**：当前发言者本人页面**不再显示远端电平**（此前会把别人的电平误读成自己的音量），只留「N号 正在发言」；`presentation/voice-levels.ts` 增 `isSelfSpeaking`。
- **验证（2026-09-23，容器内 + 真机）**：增量单测 **5 文件 60 例全过**（v2-media 16 / voice-agora 13 / frontend-v2-voice-session 18 / frontend-v2-voice-levels 7 / `v2-voice-delivery-api` 6）；服务端与三个前端 `typecheck` 通过；E2E `27-voice-delivery`（新）**双浏览器 6/6**、`18-voice-levels` **双浏览器 8/8**。**真机（真实声网凭据）**：`16-voice` **1 passed（16.6s）**——发言者页截图显示**「已送达 12/12」**（C 组在真实媒体上成立）、听众页 `5号/6号 正在发言 · 69–78%` 且**无送达行**（隐私门控成立）、撤权后无送达行；api 容器日志 **0** 条媒体失败告警。REST 探针：客户密钥在**活频道**上 `queryChannelUsers` → `{channelExist:true, mode:1, users:[1..10]}`；匿名凭据 → `HTTP 401 Invalid authentication credentials`。
- **未覆盖 / 未做**：D 的对账计数与「踢未知 uid」真实路径（验收栈未设 `ADMIN_PASSWORD`）、`publish_audio` 兜底的实网行为、增益/AGC 听感均**未验**；`16-voice` 对送达行只有截图、**无断言**；`docs/openapi-v2.2.json` 尚未补 `/voice/receipt` 路径；**本版未跑全量**；未部署。
