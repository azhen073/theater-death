...
- **未覆盖 / 未实现**：v2 入口真实媒体已验（见上）；**仍未验**——麦克风增益 150 与 AGC 的实际听感/响度、D 的对账计数与「踢未知 uid」真实路径（验收栈未设 `ADMIN_PASSWORD`）、`publish_audio` 兜底的实网行为；**客户 ID/密钥：2026-09-21 曾决定不验，2026-09-23 用户改定为「后续可以引入」**——踢人与终局关房的真实 REST 调用、v1 `02-voice` 随之转为**待引入凭据后补验**（生产要用踢人/终局关房与频道对账，`.env` 必须配客户 ID/密钥；连带待办：install 脚本生成的 `.env` 补这两项、接通 `AGORA_REST_BASE_URL`）；`16-voice` 对送达行只有截图、**无断言**；`docs/openapi-v2.2.json` 尚未补 `/voice/receipt` 路径；本版**未跑全量**。
