...
- **计划内容（实现后在此补记实测与未覆盖项）**：
  - **局内麦克风音量显示**（方案已提交待定案）：分 A / A+B / A+B+C 三档——A 只显示自己的电平；B 增加「当前发言者」电平；C 为 13 座位全员电平环（需服务端给出 uid↔座位映射，涉及契约与夹具）。共同约束：**音量是纯本地信号，不上报服务端、不入日志与复盘**；受 R-43 许可时段门控（投票/夜间等窗口不显示）；尊重显示设置里的「减少动画」；**不改变 R-43 与 R-35 的任何语义**。
