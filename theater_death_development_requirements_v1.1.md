...
- **未覆盖 / 未实现**：v2 入口真实媒体已验（见上）；**仍未验**——麦克风增益 150 与 AGC 的实际听感/响度、D 的对账计数与「踢未知 uid」真实路径（验收栈未设 `ADMIN_PASSWORD`）、`publish_audio` 兜底的实网行为；**已决定不验（用户决定 2026-09-21）**——踢人与终局关房的真实 REST 调用、v1 `02-voice`（两者需控制台「客户 ID/密钥」，不再引入；生产若要用踢人/终局关房，`.env` 仍必须配该凭据）；`16-voice` 对送达行只有截图、**无断言**；`docs/openapi-v2.2.json` 尚未补 `/voice/receipt` 路径；本版**未跑全量**。
