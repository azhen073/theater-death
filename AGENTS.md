...
- 舞台行动 UX 已整合进 main（2026-09-21，贡献 syhneversigh，阿真确认）：舞台内行动交互与竖屏适配 + 边界修复 + 团队方案“发布并确认本人”单请求原子操作（保留全队逐版确认与固定窗口，R-47 语义不变）。维护方复核后修复环形/文档流判定的竞态（加宽后最多约 1.5 秒滞后且浏览器不一致）并补回归 E2E；未部署。
