...
- **v2.0.2-beta（进行中，贡献 kiahir）**：分支 `2.0.2-beta`（由 `2.0.2-alpha` 改名）。已落地：遗言系统复核 + G5 顺序明文（R-45 补「同日多名出局者按座位号升序、每人各自 60 秒」，**无行为变更**）+ G4 遗言界面 E2E（`e2e/specs-v2/17-last-words.spec.ts`）。未决：`lastWords.firstNight`/`otherNights` 为死配置；麦克风许可仅单测层覆盖。上一条（v2.0.2-alpha）即其内容基线。
