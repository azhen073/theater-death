...
- **v2.0.3-alpha（进行中，贡献 kiahir）**：分支自 `2.0.2-beta` 的 tip（`c5df078`）拉出。**已实现局内语音音量显示与调节（A+B + 输出/输入增益）**：自己的 5 段电平、当前发言者「N号 正在发言 · X%」（输出静音时显示「已静音」但保留"谁在发言"）、输出音量 0–100 + 一键静音、麦克风增益 0–100（重开麦/换设备自动重应用）；纯本地偏好（`localStorage`），受 R-43 时段门控、不上报服务端。实测：容器内 18 例单测全过 + `typecheck:web:v2` 通过。未覆盖/未实现：语音条无 E2E、真实听感未验、座位卡电平环与每玩家音量未做。遗留：F1/F3/F4、P1/P2、`lastWords.firstNight`/`otherNights` 死配置。
