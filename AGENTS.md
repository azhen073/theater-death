...
## 声网官方文档与核对（2026-09-22）

- **唯一官方来源**：`https://doc.shengwang.cn/`（中文站）。`docs.agora.io`、`api-ref.agora.io`、`docs-preview.agora.io`、`docs.agoraio.cn`、`doc-archive.shengwang.cn` 等**一律不算官方来源**，只能当参考；结论必须回到官方站或下面的 MCP Server 复核后才可写进代码/文档。
- **官方文档 MCP Server**：`https://doc-mcp.shengwang.cn/mcp`（Streamable HTTP，服务名 `MDX Documentation Server`，无需 session id）。三个工具：
  - `search-docs { query }` — 全文搜索官方文档
  - `list-docs { category?, limit?, sortBy? }` — 按分类列文档（例：`api-ref/rtc/javascript`，约 68 篇）
  - `get-doc-content { uri }` — 取正文；URI 形如 `docs://api-reference/rtc/javascript/interfaces/iagorartcclient`、`docs://api-reference//rtc/restful/channel-management/operations/post-dev-v1-kicking-rule`
- **调用方式**：`curl.exe` / `Invoke-WebRequest` 在本机会因 Schannel 报 `SEC_E_NO_CREDENTIALS`；改用 Node 内置 `fetch` POST JSON-RPC（`initialize` → `tools/list` → `tools/call`）即可，临时脚本用完删除。
- **已知语料缺口**：REST「查询用户列表 / 查询用户状态」（`channel-management/operations/get-*`）**不在语料里**（`get-doc-content` 返回 not found），且官方站这些页面是 JS 渲染；`kicking-rule` 系列、Web SDK 客户端 API（属性/事件/字段）都在语料中，可直接核对。遇缺口时用浏览器打开官方站对应页人工确认并回填。
- **纪律**：涉及声网实现的事实（API 名、字段名、错误码、限制、调用时机）**先查 MCP 再写码**，并在文档中标注出处；官方未覆盖的行为不得凭非官方站推断。示例见 `docs/proposal-voice-delivery-observability.md`（逐条标注核对状态）。
...
- **v2.0.6-alpha 完成（贡献 kiahir，阿真确认；已合并进 main `20ccd06`，保留贡献提交 `f40a1ee`；原分支 `2.0.4-beta` 于 2026-09-22 改名）**：维护方复核（2026-09-23）：全量 **93 文件 643 例全过** + 增量 6 文件 74 例 + 4 typecheck + E2E `27-voice-delivery`/`18-voice-levels` 双浏览器 14/14 + 真实凭据 `16-voice` chromium 1/1（16.4s）；合并后小修：install.ps1 恢复 BOM、无客户凭据时不暴露 `queryChannelUsers`（对账按 skipped，与 RUNBOOK 一致）、文档笔误。贡献方记录：分支自 `main` 的 `9c86a90` 拉出（版本名 = 分支名），并**已合并 `origin/main` 的 `3379726`**（v2.0.5-alpha 开局身份揭示 + UI 优化批次 PR #9–#16）。声网语音审计后的**可靠性收口**（无玩法/契约字段变更）：① `server/v2/media.ts` 踢出改为**成功后才删身份映射**（失败留给下一次 `sync` 重试，逐身份捕获失败，`required` 时抛错映射 `/voice/sync` 503），`enqueue` 队列链**永不 reject**（修掉「`required` 失败后下一次入队的 work 被跳过」）；② `voice/agora.ts` 频道管理 REST 加 `AbortSignal.timeout`（15s）+ 5xx/超时/网络错误退避重试（3 次尝试，250ms→1s），4xx 不重试；③ 发布凭证 TTL **600s → 150s**（最长发言窗口 120s + 30s 缓冲；加入凭证仍 1800s，不影响收听）——「撤销强度」取方案①，方案②（失去发布权即踢 uid）待定案；④ 客户端（v2 `session.ts`/`bar.tsx` + v1 `voice.tsx`）补 `token-privilege-will-expire`/`did-expire`（过期前续期 / 过期后重连并恢复开麦）、autoMic 只在 `connected` 才记窗口键、重连期间开麦按钮禁用 +「重连中…」、`#stopTask` 串行化（关麦立刻重开不双发）、uid 归属与 `exception` 提示、离开复位 `onAutoplayFailed`、设备选择会话内保留；⑤ **端到端送达回执（C）**：`POST /api/v2/rooms/:code/voice/receipt`（限流 60/分 + 成员/对局校验，身份取会话解析、不信任载荷）+ `private.voice.delivery` **只下发给当前发言者**，界面 10px 灰字「已送达 N/M」（听众为 0 不显示）；⑥ **频道轮询对账（D，不使用 NCS）**：`queryChannelUsers` + `media.reconcile` 每 5 秒仅对进行中对局比对频道实况，**只踢未知 uid**（听众合法在线，不得误踢），计数经 `ADMIN_PASSWORD` 的 `AdminSummary.voice.reconcile` 暴露；⑦ **发言者本人页面不再显示远端电平**（只留「N号 正在发言」）；⑧ **部署面凭据链补齐**（install 脚本生成 `.env` 补客户 ID/密钥 + `ADMIN_PASSWORD` + `AGORA_REST_BASE_URL`；主 `docker-compose.yml` 此前**根本没注入 `ADMIN_PASSWORD`**、`compose.v2.release.yml` 只注入 `VOICE_ENABLED` → 均已补）；⑨ **缺凭据降级不崩**（`VOICE_ENABLED=true` 缺 App ID/证书 → 文字测试模式 + 告警；凭据格式非法仍拒绝启动）与**空 `AGORA_REST_BASE_URL` 修复**（空串会让 REST 变成相对 URL，踢人/关房/对账全废、对账只涨 `failures`）；⑩ **关房改为「官方调用 + 逐个补踢」**：不带 uid 的踢人规则实测返回 `success/id 0` 却一个人都不踢（静默空操作），补踢后终局/解散才能真正清空频道，残留打 `voice_close_room_incomplete`；对账失败也补打 `voice_reconcile_query_failed <原因>`；方案、出处与逐条官方核对见 `docs/proposal-voice-delivery-observability.md`。**实测**：增量 **5 文件 60 例全过**（先前一批为后端 5 文件 40 例 + 客户端 2 文件 19 例；C/D 后为 v2-media 16 / voice-agora 13 / frontend-v2-voice-session 18 / frontend-v2-voice-levels 7 / `v2-voice-delivery-api` 6；客户 ID/密钥补验后 voice-agora 16、v2-config 10）；四个 typecheck 与 E2E `18-voice-levels`（**双浏览器 8/8**）、`27-voice-delivery`（**双浏览器 6/6**）通过，镜像重建（构建链内含**全量单测 + 4 typecheck + 双前端构建**）通过；stash 旧实现对照 **2 / 6 / 1 failed**；**真机 `16-voice` 1 passed（12.3s 一次、C/D 后 16.6s 一次）**：接收方远端电平 35–42% / 69–78%，发言者页「已送达 12/12」、听众页无送达行；**真实凭据补验（2026-09-23）**：D 对账 `rounds` 0→4→8→16→24、`failures` 0、未知 uid（自签 uid=0 通配凭证）被踢出、踢人 REST（555001 离开而 555002 留下）、关房修复后频道清空、**v1 `02-voice` 2/2（1.1 分钟）**；静态清点（合并前基线 `9c86a90`）88 文件 584 例、v2 E2E 18 spec / 53 例；**合并 `origin/main` 后**静态清点 92 文件 / 606 例、v2 E2E **28 spec / 90 例**（合并进 main 后维护方实测全量 93 文件 643 例全过，见上）。**未单独复跑全量**（镜像构建链内含全量单测与四个 typecheck 通过）；仍未验：增益/AGC 听感、`publish_audio` 兜底（按决定只作兜底、未启用）、`16-voice` 对送达行无断言（仅截图）；**踢人/关房真实 REST 与 v1 `02-voice` 已于 2026-09-23 凭据到位后补验通过**（生产要用踢人/关房/对账，`.env` 必须配客户 ID/密钥）。**已知风险（已接受，用户决定 2026-09-21）**：实测并复测本项目**「连麦鉴权」未生效** → 订阅 token 在 rtc 与 live 下都能发麦、20s 的 `pubAudio` 过期后仍能发麦，即声网侧不强制；缓解 = 服务端签发/撤回（依赖客户端 `renewToken`）+ 发布 TTL 150 秒上限；将来若要强保证，可启用该功能或改用方案②（失去发布权即踢 uid）。仍余 1 条未做：v1 无自动重连且 uid = 座位号（多标签页 `UID_CONFLICT`）。审计 9 项中 **②③④⑤ 已在本版修复**（缺凭据降级、release compose 注入 `AGORA_*`、install 脚本补客户 ID/密钥与 `ADMIN_PASSWORD`、`AGORA_REST_BASE_URL` 双端接通且空值按未配置处理），其余未处理：openapi 的 voice/token 仍是 LiveKit 形态、v1 不回收玩家媒体、文档过期、token 角色/权限位无断言、首次安装 `NODE_ENV` 相冲；见需求文档文末 v2.0.6-alpha 一节。
