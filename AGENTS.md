...
- **v2.0.4-beta（进行中，贡献 kiahir）**：分支自 `main` 的 `9c86a90` 拉出（版本名 = 分支名）。声网语音审计后的**可靠性修复**（无玩法/契约字段变更）：① `server/v2/media.ts` 踢出改为**成功后才删身份映射**、失败留给下一次 `sync` 重试（声网一次性踢出在断连时会失败，官方要求重调），`sync` 逐身份捕获失败并统计；② 同文件 `enqueue` 的队列链**永不 reject**，修掉「`required` 失败后下一次入队的 work 被跳过」导致的静默丢任务；③ `voice/agora.ts` 频道管理 REST 加 `AbortSignal.timeout`（默认 15s）与 5xx/超时/网络错误的退避重试（默认 3 次尝试，250ms→1s），4xx 不重试。实测：增量 **5 文件 39 例全过** + 服务端 `typecheck` 通过；并**把 `media.ts` 临时 stash 回旧实现验证两条新用例确实失败**（回归有效）。静态清点 88 文件 577 例；**未跑全量**、真实媒体仍未验。**待定案**：撤销强度（v2 无 token 推送，被改客户端可在窗口结束后继续发麦至多 600 秒；候选：缩短发布 TTL / 失去发布权即踢 uid / 仅文档明示）。审计另发现但**本版未处理**的项（openapi 的 voice/token 响应仍是 LiveKit 形态、v2 缺凭据时崩溃而非降级、`compose.v2.release.yml` 不注入 `AGORA_*`、install 脚本漏客户密钥、`AGORA_REST_BASE_URL` 服务端未接通、v1 不回收玩家媒体、autoMic 与重连叠加、token 角色/权限位无断言）见需求文档文末 v2.0.4-beta 一节。
