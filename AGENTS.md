...
- **v2.0.4-beta（进行中，贡献 kiahir）**：分支自 `main` 的 `9c86a90` 拉出（版本名 = 分支名）。声网语音审计后的**可靠性修复**（无玩法/契约字段变更）：① `server/v2/media.ts` 踢出改为**成功后才删身份映射**、失败留给下一次 `sync` 重试（声网一次性踢出在断连时会失败，官方要求重调），`sync` 逐身份捕获失败并统计；② 同文件 `enqueue` 的队列链**永不 reject**，修掉「`required` 失败后下一次入队的 work 被跳过」导致的静默丢任务；③ `voice/agora.ts` 频道管理 REST 加 `AbortSignal.timeout`（默认 15s）与 5xx/超时/网络错误的退避重试（默认 3 次尝试，250ms→1s），4xx 不重试；④ **撤销强度按方案①收口**（同文件）：发布凭证默认 TTL 由 600 秒收紧为 **150 秒**（最长 120 秒发言窗口 + 30 秒缓冲），把「客户端不配合 `renewToken` 时窗口外仍能发麦」的最坏时长压到 2.5 分钟内（方案②「失去发布权即踢 uid」本轮不做）。实测：增量 **5 文件 40 例全过**（含默认 TTL 上限用例）+ 服务端 `typecheck` 通过；并**把 `media.ts` 临时 stash 回旧实现验证两条新用例确实失败**（回归有效）。静态清点 88 文件 578 例；**未跑全量**、真实媒体仍未验。审计另发现但**本版未处理**的项（openapi 的 voice/token 响应仍是 LiveKit 形态、v2 缺凭据时崩溃而非降级、`compose.v2.release.yml` 不注入 `AGORA_*`、install 脚本漏客户密钥、`AGORA_REST_BASE_URL` 服务端未接通、v1 不回收玩家媒体、token 角色/权限位无断言）见需求文档文末 v2.0.4-beta 一节。
- **v2.0.4-beta 追加②（客户端连接可靠性，2026-09-21）**：以 SDK 官方 typings 为据复核 v2（`web-v2/.../voice/session.ts`、`bar.tsx`）与 v1（`web/src/voice.tsx`），修 5 类：① 补 `token-privilege-will-expire`（过期前 30 秒 `renewToken`）与 `token-privilege-did-expire`（过期后重新 `join` 并恢复开麦）——此前两个客户端都没订阅，纯听众 30 分钟后必然掉线、v1 更无自动重连；② `bar.tsx` 原先在 `reconnecting` 也记「自动开麦」的窗口键（而 `requestMicrophone()` 在非 `connected` 时静默返回）→ 重连期间轮到自己会**整个窗口都不自动开麦**，改为只有 `connected` 才记键；③ 重连期间开麦按钮改为禁用 +「重连中…」；④ `#stopTrack()` 引入 `#stopTask` 串行化：关麦后立刻重开不再短暂双发（自听回声）；⑤ 电平按 `client.join()` 返回的 uid 归属、离开时复位 `AgoraRTC.onAutoplayFailed`、订阅 `exception` 把音频质量事件码写成界面提示、设备选择会话内保留。实测：单测 **19 例全过**（voice-session 13 + voice-levels 6，新增 6 例）、`typecheck:web:v2` / `v2-tests` / `web`(v1) 三工程通过、E2E `18-voice-levels` **chromium 3/3 + webkit 3/3**；并用「stash 回旧实现」验证新用例确实有牙（旧 `session.ts` → 6 failed / 7 passed；旧 `bar.tsx` → 1 failed）。静态清点更新为 **88 文件 584 例**、v2 E2E 静态 **18 spec / 53 例**。仍余 1 条未做：**v1 无自动重连且 uid = 座位号**（多标签页会 `UID_CONFLICT`），按约定不动。
- **v2.0.4-beta 追加·声网鉴权强制力实测（发现「连麦鉴权」未开启，2026-09-21）**：用服务端自签 token 在真实频道上做探针（rtc 与 live 两种 mode）测出——**加入强制校验 token**（无 token → `dynamic use static key`；错证书 → `invalid token, authorized failed`）✅；但**发布权限位当前无约束力**：持订阅角色 token 在 rtc 与 live（`audience`→`host`）下都**成功 publish**，且 `pubAudio` 仅 20 秒的凭证 35 秒后仍能再发麦 → 按官方文档可判定该项目的**「连麦鉴权」尚未开启**。**结论：R-43 的发布权目前只由我们服务端签发/撤回（依赖客户端 `renewToken`），声网侧不兜底**；发布 TTL 150 秒就是它的上限兜底。**部署前必须在控制台「项目 → 编辑 → 功能 → 连麦鉴权」启用**（约 5 分钟生效），开启后应重跑探针确认订阅 token 的 `publish` 变为 rejected。另注：此前「真实云联调 3 例全过」只断言频道在线+界面文案，**结构上查不出这一项**，属长期验证盲区；探针为临时用例，验证后已删除。
- **v2.0.4-beta 追加③（声网真机验收通过，2026-09-21）**：用真实声网凭据（本机 `.env`，不入库）跑通 `16-voice`：13 客户端真机加入频道、候选开麦发布、**接收方远端电平 35–42%（真实收到音频）**、无自动播放拦截、结束发言即时撤权 —— **1 passed（12.3s）**。**「真实媒体未验」这一条对 v2 入口已经结清**（v1 `02-voice` 仍需控制台「客户 ID/密钥」）。该 spec 有两处自声网迁移起就坏掉的缺陷，一并修好：① 文案定位器被 v2.0.3-alpha 新增的 `.voice-bar__speaker`（「N号 正在发言 · X%」）撞成 strict mode 双命中 → 收窄到 `.voice-bar__status span`；② 原「收到远端音频」断言查 `<audio>` 元素，而 **Agora Web SDK v4 用 WebAudio 播放、页面无 audio 元素**（实测 `audioCount=0`）→ 改为断言远端电平 > 0 + 无「点击启用声音」。
