...
2026-09-19 退出对齐：`POST /rooms/:code/leave` 保留原请求与响应格式，退出房间不注销账号。局中正式玩家显示“暂离对局”，本局席位保留，对局继续计时。复盘页提供直接“离开房间”。**2026-09-20 起**：房主在**大厅/复盘**退出即解散整房（响应带 `dissolved:true`），房主在**对局中**退出仍是“暂离”并由其他在线正式成员按规则继任；房主另可在**任意阶段**主动解散（`POST /rooms/:code/dissolve`，仅房主；对局中会立即终止本局并按 `aborted` 记账）。复盘阶段无正式成员时立即关闭房间（含剩余观众），最后退出者的 `seatRetained=false`；单纯离线仍计为正式成员，但**全员离线满 24 小时**房间会被回收。大厅与进行中阶段无正式成员仍按5分钟清理。房主“结束复盘，返回大厅”是独立操作，保留原房间供下一局使用。完整对比见 [房间退出对齐](frontend-v2-room-exit.md)。
