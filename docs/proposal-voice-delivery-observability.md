...
- 提案人：kiahir ｜ 状态：**C 组（送达回执）与 D 组（频道轮询对账）已于 2026-09-23 落地并验收**；A/B 组经复核放弃，其余为备用方案（见需求文档文末 v2.0.6-alpha 一节）
...
| **D** | 声网侧硬证据 → **详设见 §12**：只做 **D-a 轮询对账**（`queryChannelUsers` / `queryUserStatus`）；**不使用 NCS 事件回调（用户决定 2026-09-22）** | 不改契约；配套清理可选的 webhook/LiveKit 残留 | **客户 ID/密钥**（2026-09-21 曾决定不引入；需重新裁定）+ 区域 base URL | 中 | 暂缓/待定（拿不到凭据就不做） |
...
5. **D 组是否重启凭据**：是否重新引入控制台「客户 ID/密钥」（另加 `AGORA_REST_BASE_URL`），从而启用服务端强制对账？
...
- **与既有决定冲突**：需要引入并使用控制台「客户 ID/密钥」；2026-09-21 已决定"不再引入"，本方案不擅自改变，只给出"若要强保证"的完整路径
