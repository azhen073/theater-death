...
- **踢人 / 关房的可靠性（v2.0.4-beta）**：频道管理 REST（`POST /dev/v1/kicking-rule`，`time: 0` 一次性踢出、不带 uid 即"解散频道"）现在每次请求带 15 秒超时，5xx / 超时 / 网络错误按 250ms→1s 退避重试（默认最多 3 次尝试），4xx 视为永久失败不重试；服务端**踢成功后才删除该身份的 uid 映射**，失败的留给下一次状态同步重试（声网官方说明：一次性踢出在对方当时与边缘节点断开时会失败，需要再次调用）；同一局的媒体副作用串行执行且队列不会因一次失败而跳过后续操作。**注**：一次性踢出按设计不拉黑——被踢者可用（有效期内的）订阅凭证立即重进，这与"踢人 = 清位"的既有语义一致。
- **发布凭证有效期（v2.0.4-beta 收紧）**：发布凭证默认 TTL 由 10 分钟改为 **150 秒**（最长发言窗口 120 秒 + 30 秒缓冲）。这是"服务端无法实时降权"的补偿手段：客户端不配合 `renewToken` 时，窗口结束后最多还能发 150 秒（原为 600 秒）。加入凭证仍是 30 分钟，**收听/订阅不受影响**（到期只收回发布权，不会断开会话）。
- **凭证续期与重连（v2.0.4-beta）**：两个客户端都订阅了 SDK 的 `token-privilege-will-expire`（过期前 30 秒，按官方建议重新取 token 并 `renewToken`）与 `token-privilege-did-expire`（过期后重新 `join`，原本正在发言则自动恢复开麦；v1 保留观战身份）。**加入凭证 TTL 为 30 分钟，长局必然遇到续期点**，此前没有任何处理、纯听众会被静默断开（v1 需手动重新点「加入语音」）。另外：「轮到我发言时自动开麦」只在连接真正处于 `connected` 时才消耗"本窗口已自动开麦"的机会——重连期间轮到自己不会白白用掉这次机会；重连期间开麦按钮禁用并显示「重连中…」。
- **可排障信号（v2.0.4-beta）**：订阅 SDK 的 `exception` 事件，把音视频**质量**异常码（2001 麦克风输入音量过低 / 2002 远端音量过低 / 2003 发送音频码率过低 / 2005 接收音频解码失败，及恢复项 4001/4002/4003）转成语音条上的一句提示；视频类异常码忽略。自己的电平按 `client.join()` 返回的 uid 归属；离开时复位全局 `AgoraRTC.onAutoplayFailed`；所选麦克风设备在会话内保留（离开再加入不回到系统默认）。**未做**：v1 客户端仍无自动重连且 Agora uid = 座位号（同账号多标签页会 `UID_CONFLICT`）。
