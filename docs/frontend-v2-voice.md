...
| 麦克风增益 | 0–150 滑杆，仅在已开麦时出现；**>110 时关闭 AGC** 并显示「AGC 已关闭」 | 只影响别人听到的音量；每次重新开麦都会新建采集轨道，**会话会记住增益并自动重新应用**（换设备同理） |
...
- **AGC 与增益的关系**：建轨时带 `AEC` / `ANS` / `AGC`。增益 ≤110 保留 `AGC: true`（自动把电平拉向目标区间）；**增益 >110 时建轨改为 `AGC: false`**，避免自动增益把手动放大压回。由于 AGC 是建轨参数、运行期没有可靠开关，**跨过 110 这条线时会重建采集轨道**（在滑杆松手提交时重建，不在拖动过程中反复重建以免断音）。两者叠加，因此「增益 150」不是线性的听感翻倍。
