...
export function publishedState(state: GameState, events: readonly GameEvent[]): GameState {
  ...
  for (const event of events) {
    ...
    const p = event.payload as {
      ...
      seat?: number;
      targetSeat?: number;
      reveals?: { seat: number; roleId: RoleId }[];
    };
    if (event.type === 'deaths_announced') {
      for (const seat of p.seats ?? []) alive.set(seat, revived.get(event.dayNumber)?.has(seat) ?? false);
    }
    ...
    if (event.type === 'reveal_announced') {
      for (const reveal of p.reveals ?? []) revealed.add(reveal.seat);
    }
  }
  return {
    ...state,
    players: state.players.map((p) => ({
      ...p,
      life: alive.get(p.seat) ? 'alive' : 'dead',
      revealed: revealed.has(p.seat),
      voteFrozen: p.roleId === 'laike' && revealed.has(p.seat) && state.stage === 1,
    })),
  };
}
